...
class Bookmarks: Fragment(R.layout.fragment_bookmarks) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
       ...
       val userEmail = auth.currentUser!!.getEmail().toString()
        ...
        val deleteall=view.findViewById<Button>(R.id.deleteall)

        deleteall.setOnClickListener {

            while (contentModel.size>0){
                contentModel.clear()
                rVadapter.notifyDataSetChanged()//데이터 변경 최신화 동기화해주기
            ...
            val iterator1 = keyModel.iterator()
            while (iterator1.hasNext()) {
                val i = iterator1.next()
                myBookmark.child(auth.currentUser?.uid.toString()).child(i).removeValue()
                iterator1.remove() // 안전하게 리스트에서 삭제
            }
            val iterator2 = contentModel.iterator()



            Toast.makeText(context, "전체삭제 완료", Toast.LENGTH_SHORT).show()
